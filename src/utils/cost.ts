...
export function getUpgradeCost(level: number, star: number): bigint {
  ...
  switch (star) {
    ...
    case 16:
      ...
      break;
    case 17:
      cost = 1000 + (Math.pow(level, 3) * Math.pow(star + 1, 2.7)) / 150;
      break;
    case 18:
      cost = 1000 + (Math.pow(level, 3) * Math.pow(star + 1, 2.7)) / 70;
      break;
    case 19:
      cost = 1000 + (Math.pow(level, 3) * Math.pow(star + 1, 2.7)) / 45;
      break;
    case 20:
      cost = 1000 + (Math.pow(level, 3) * Math.pow(star + 1, 2.7)) / 200;
      break;
    case 21:
      cost = 1000 + (Math.pow(level, 3) * Math.pow(star + 1, 2.7)) / 125;
      break;
    case 22:
    case 23:
    case 24:
...
export function getActualCost(data: DataForCost): bigint {
  ...
  let applyPenalty: boolean =
    isPreventableStar(data.currentStar) &&
    data.destroyPercent > 0 &&
    data.preventDestroy;
  let penaltyCost: bigint = applyPenalty
    ? data.originalCost * BigInt(2)
    : BigInt(0);
  // mvp 혜택에 의한 할인율 먼저 구해주기
  let discountRate: number =
    data.mvpRank === MVPRank.bronze
      ? BRONZE_DISCOUNT_RATE
      : data.mvpRank === MVPRank.silver
      ? SILVER_DISCOUNT_RATE
      : data.mvpRank === MVPRank.gold
      ? GOLD_DISCOUNT_RATE
      : DIA_DISCOUNT_RATE;
  ...
  let actualCost: bigint =
    (data.originalCost *
      BigInt(100 - discountRate) *
      BigInt(100 - eventDiscountRate)) /
      BigInt(10000) +
    penaltyCost;
