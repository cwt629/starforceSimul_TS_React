...
import {
  getExpectationByStarcatch,
  getReinforceResult,
  isPreventableStar,
} from "../utils/reinforce";
...
function Simulator() {
  ...
  const currentStar: number = useSelector(
    (state: RootState) => state.currentStar
  );
  const maxStar: number = useSelector((state: RootState) => state.maxStar);
  const successPercent: number = useSelector(
    (state: RootState) => state.successPercent
  );
  const destroyPercent: number = useSelector(
    (state: RootState) => state.destroyPercent
  );
  const noStarcatch: boolean = useSelector(
    (state: RootState) => state.noStarcatch
  );
  const preventDestroy: boolean = useSelector(
    (state: RootState) => state.preventDestroy
  );
  ...
  const restoreCost: bigint = useSelector(
    (state: RootState) => state.restoreCost
  );
  const totalSuccess: number = useSelector(
    (state: RootState) => state.totalSuccess
  );
  const totalFailure: number = useSelector(
    (state: RootState) => state.totalFailure
  );
  const totalDestroy: number = useSelector(
    (state: RootState) => state.totalDestroy
  );
  const totalSpent: bigint = useSelector(
    (state: RootState) => state.totalSpent
  );
  ...
  // 자동 강화 속도 옵션들
  const autoSpeedOptions: AutoOption[] = [
    { interval: 1000, name: "매우 느리게" },
    { interval: 500, name: "느리게" },
    { interval: 200, name: "보통" },
    { interval: 100, name: "빠르게" },
    { interval: 50, name: "매우 빠르게" },
  ];
  ...
  useEffect(() => {
    if (achieved && !autoSaved) {
      ...
      finishAndGetTitle(currentLog).then((title: string) => {
  ...
  const handleReinforceClick = () => {
    ...
    let currentExp: ResultExpectation = {
      ...
      failure: 0,
      destroy:
        isPreventableStar(currentStar) && preventDestroy ? 0 : destroyPercent, // 파괴방지 적용
    };
    currentExp.failure = 100 - currentExp.success - currentExp.destroy;

    // 실제로 적용될 확률 정보
    let finalExp: ResultExpectation = getExpectationByStarcatch(
      currentExp,
      !noStarcatch
    );

    const result: Result = getReinforceResult(
      finalExp.success,
      finalExp.destroy
    );
    switch (result) {
    ...
    <div className="simulator">
      <ul className="nav nav-tabs nav-justified simul-tabs">
        <li className="nav-item">
          <span
            className={`nav-link ${autoMode ? "" : "active"} ${
              autoIntervalID ? "disabled" : ""
            }`}
        ...
        <li className="nav-item">
          <span
            className={`nav-link ${autoMode ? "active" : ""} ${
              achieved || autoIntervalID ? "disabled" : ""
            }`}
        ...
        <div className="simul-ingame">
          <div className="auto-option-block">
            <div className="simul-options option-ingame">
              ...
              <div className="form-check">
                ...
                />
                <label className="form-check-label" htmlFor="starcatch">
                  스타캐치 해제
                </label>
              </div>
              <div className="form-check">
                ...
                />
                <label className="form-check-label" htmlFor="destroyShield">
                  파괴방지
                </label>
              </div>
            </div>
            <div className="input-group">
              ...
              >
                {autoSpeedOptions.map((option, i) => (
                  <option key={i} value={option.interval}>
                    {option.name}(1초당 {(1000 / option.interval).toFixed(1)}회)
                  </option>
                ))}
              </select>
            </div>
            ...
            {autoIntervalID ? (
              <div className="auto-prevention-block">
                ...
                <div className="auto-prevention-text">
                  자동 강화 진행 중{" "}
                  <span className="spinner-border text-primary auto-spinner"></span>
                </div>
              </div>
            ) : (
              <></>
          ...
          {autoIntervalID ? (
            <button
              ...
              onClick={handleAutoPauseClick}
            >
              강화 중지 <i className="bi bi-pause-fill"></i>
            </button>
          ) : (
            <button
              type="button"
              className={`btn btn-primary ${achieved ? "disabled" : ""}`}
              onClick={handleAutoStartClick}
            >
              강화 시작 <i className="bi bi-caret-right-fill"></i>
            </button>
          )}
        </div>
      ) : (
            ...
            <div className="simul-options option-ingame">
              ...
              <div className="form-check">
                <input
                  ...
                  onChange={(e) => handlePreventDestroy(e)}
                  disabled={
                    destroyPercent === 0 || !isPreventableStar(currentStar)
                  }
                />
                <label className="form-check-label" htmlFor="destroyShield">
                  파괴방지
