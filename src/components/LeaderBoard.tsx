...
function LeaderBoard() {
  ...
  const failurePercent: number = useSelector(
    (state: RootState) => state.failurePercent
  );
  ...
  const bonusUnderTen: boolean = useSelector(
    (state: RootState) => state.bonusUnderTen
  );
  const eventDC30: boolean = useSelector((state: RootState) => state.eventDC30);
  const successOnFives: boolean = useSelector(
    (state: RootState) => state.successOnFives
  );
  const reduceDestroy: boolean = useSelector(
    (state: RootState) => state.reduceDestroy
  );
  ...
  const MVPRankName: string =
    mvpRank === MVPRank.bronze
      ? "브론즈"
      : mvpRank === MVPRank.silver
      ? "실버"
      : mvpRank === MVPRank.gold
      ? "골드"
      : mvpRank === MVPRank.dia || mvpRank === MVPRank.red
      ? "다이아/레드"
      : "불명";
  ...
  if (reduceDestroy) appliedEventNames.push("21성 이하 파괴 확률 30% 감소");
      ...
      <table className="table table-bordered setting-table">
        <tbody>
          ...
          <tr>
            <td>목표</td>
            <td>
              {start}성 {">"} {goal}성 {achieved ? <b>(달성!)</b> : ""}
            </td>
          </tr>
          <tr>
            <td>장비 복구 비용</td>
              ...
              <ul>
                ...
                <li>
                  ...
                  {appliedEventNames.length > 0 ? (
                    <ul>
                      {appliedEventNames.map((eventName, index) => (
          ...
          <tr>
            ...
            <td>
              <b>{totalSpent.toLocaleString()}메소</b>
            </td>
          </tr>
        </tbody>
      </table>
            ...
            <td colSpan={3}>
              <div>
                ...
                <div className="upgrade-info">
                  ...
                  {currentStar < maxStar ? (
                    <>
                      강화 비용: {cost.toLocaleString()}메소
                      <br />
                      성공 확률: {successPercent.toFixed(1)}%<br />
                      실패({ableToFall ? "하락" : "유지"}) 확률:{" "}
                      {failurePercent.toFixed(1)}%<br />
                      {destroyPercent > 0
                        ? `파괴 확률: ${destroyPercent.toFixed(1)}%`
                        : ``}
                    </>
                  ) : (
        ...
        <div className="modal-dialog">
          <div className="modal-content">
            <div className="modal-header-custom">
              ...
              <button
                type="button"
                className="btn-close modal-header-close"
