...
const simulSlice = createSlice({
  ...
  reducers: {
    ...
    init: (state, action: PayloadAction<InitialData>) => {
      ...
      state.originalCost = getUpgradeCost(
      ...
      if (state.currentStar < state.maxStar) {
        state.successPercent =
          reinforceData.percentage[action.payload.start].success;
        state.destroyPercent =
          reinforceData.percentage[action.payload.start].destroy;
    ...
    grantSuccess: (state) => {
      ...
      state.nextStar =
        state.bonusUnderTen && state.currentStar <= 10
          ? state.currentStar + 2
          : state.currentStar + 1;
